...
export default async function DashboardPage() {
  ...
  const filteredSuppliers = suppliers.filter(s => {
    ...
    // Category restriction (if I have more granular data, I'd use it here)
      ...
      <div className="pt-28 pb-24 px-4 md:px-8 max-w-[1600px] mx-auto animate-in fade-in duration-700">
        ...
        <div className="mb-12">
          ...
          <p className="text-gray-400 text-sm md:text-base mb-8 max-w-2xl">
            Discover winning products, analyze profitability, and instantly connect with high-quality, verified manufacturers.
          </p>
          ...
          <div className="relative group max-w-3xl">
            ...
            <input 
              type="text" 
              placeholder="Search by niche, product, or supplier name..." 
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-10">
          {[
            { label: "Total Suppliers", value: stats.suppliers.toString(), icon: Users },
            { label: "Saved Favorites", value: stats.favorites.toString(), icon: Star },
            { label: "Platform Avg. MOQ", value: "75", icon: Package },
            { label: "Top Trending Niche", value: "Electronics", icon: Zap, highlight: true },
          ].map((stat, i) => (
            <div
              key={i}
            ...
            {features.access.suppliers === 'platforms' && (
              <div className="mt-8 p-6 bg-red-500/5 border border-red-500/20 rounded-3xl relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-sm font-bold mb-2">Direct Contacts Locked</h3>
                  <p className="text-xs text-gray-400 mb-4 leading-relaxed">Upgrade to Importateur to unlock direct manufacturer phone numbers and WeChat IDs.</p>
          ...
          <main className="min-w-0 flex flex-col h-full">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              <h2 className="text-lg font-semibold flex items-center gap-2">
                Discovery Grid
                <span className="bg-white/10 text-xs px-2 py-0.5 rounded-md text-gray-400 font-normal">{filteredSuppliers.length} Results</span>
              </h2>
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <button className="flex-1 sm:flex-none flex items-center justify-between gap-2 bg-white/5 hover:bg-white/10 border border-white/10 px-4 py-2 rounded-xl text-sm font-medium transition-colors">
                  Sort by: Recommended <ChevronDown size={14} className="text-gray-400" />
                </button>
              </div>
            </div>
            ...
            <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-6">
              ...
              {lockedCount > 0 && Array.from({ length: 3 }).map((_, i) => (
                <div key={`locked-supplier-${i}`} className="bg-white/5 border border-dashed border-white/10 rounded-[2rem] h-[280px] flex flex-col items-center justify-center p-8 text-center opacity-60">
                   ...
                   <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Locked Supplier</p>
                   <p className="text-xs text-gray-500 leading-relaxed">Upgrade to see premium direct contacts and manufacturers.</p>
                </div>
              ))}
            </div>
